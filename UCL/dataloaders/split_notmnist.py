...
#     mean = (0.1307,)
#     std = (0.3081,)
    if not os.path.isdir('../data/binary_split_notmnist/'):
        os.makedirs('../data/binary_split_notmnist')
        root = os.path.dirname(__file__)
        data = split_notMNIST_loader(os.path.join(root, '../../data/notMNIST_large'))
        
        for i in range(5):
            for s in ['train', 'test']:
                data[i][s]['X'] = torch.stack(data[i][s]['X'])
                data[i][s]['y'] = torch.LongTensor(np.array(data[i][s]['y'], dtype=int)).view(-1)
                torch.save(data[i][s]['X'],os.path.join(os.path.expanduser('../data/binary_split_notmnist'),
                                                        'data' + str(i) + s + 'x.bin'))
                torch.save(data[i][s]['y'],os.path.join(os.path.expanduser('../data/binary_split_notmnist'),
                                                        'data' + str(i) + s + 'y.bin'))
    else:
        # Load binary files
        for i in range(5):
            ...
            for s in ['train', 'test']:
                data[i][s] = {'X': [], 'y': []}
                data[i][s]['X'] = torch.load(os.path.join(os.path.expanduser('../data/binary_split_notmnist'),
                                                          'data' + str(i) + s + 'x.bin'))
                data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser('../data/binary_split_notmnist'),
                                                          'data' + str(i) + s + 'y.bin'))
