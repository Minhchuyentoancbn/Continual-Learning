...
for iteration in range(int(100 / nb_cl)):
    ...
    else:  # Update only the weights of the last layer
        optimizer = optim.SGD(network.linear.parameters(), lr=lr_old, momentum=0.9)
    ...
    np.save('top1_acc_list_cumul_fixedrepr_cl' + str(nb_cl), top1_acc_list_cumul)
    ...
    # Save the network
    torch.save(network.state_dict(), 'models/network' + str(iteration + 1) + '_of_' + str(int(100 / nb_cl)) + '_fixedrepr.pt')
    ...
    for batch in iterate_minibatches(X_valid_cumul, y_valid_cumul, min(500, len(X_valid)), shuffle=False):
        ...
        with torch.no_grad():
            ...
            _, predicted = torch.max(outputs, 1)
            stat_fixedrepr.append((predicted == targets_prep).sum().item() / inputs.shape[0])
        
    print('  cumulative accuracy: \t\t\t{:.2f} %'.format(np.mean(stat_fixedrepr) * 100))
    top1_acc_list_cumul[iteration] = np.mean(stat_fixedrepr) * 100

    # Freeze the weights of the network except for the last layer:
    for param in network.parameters():
