...
for iteration in range(int(100 / nb_cl)):
    ...
    # Save the network
    torch.save(network.state_dict(), 'models/network' + str(iteration + 1) + '_of_' + str(int(100 / nb_cl)) + '_finetuning.pt')
    ...
    for batch in iterate_minibatches(X_valid_cumul, y_valid_cumul, min(500, len(X_valid)), shuffle=False):
        ...
        with torch.no_grad():
            ...
            _, predicted = torch.max(outputs, 1)
            stat_finetuning.append((predicted == targets_prep).sum().item() / inputs.shape[0])
